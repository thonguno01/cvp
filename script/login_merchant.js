...
function form_validate() {
    ...
    if (flag == true) { 
        ...
        let checkLogin = callAjax('/merchant-login-success', 'post', data_json);
        if (checkLogin.rs == 'authentic') {
            window.location = '/email-authentic';
            alert(checkLogin.msg);
        }
        else if (checkLogin.rs == 'authentic_food') {
            alert(checkLogin.msg + " à bạn chưa hoàn thiện hồ sơ. Hoàn thiện ngay!")
            window.location = '/register-food';
        }
         else {
            if(checkLogin.rs == false){
