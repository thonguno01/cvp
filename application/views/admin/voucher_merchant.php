  ...
  <div class="content-wrapper">
      ...
      <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1 class="m-0">Quản lý voucher của cửa hàng</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="admin-index">Home</a></li>
                <li class="breadcrumb-item active">Quản lý Voucher</li>
                <li class="breadcrumb-item active">Voucher của cửa hàng</li>
              </ol>
            </div><!-- /.col -->
          </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    ...
    <section class="content">
        <div class="row">
          <form action="" method="post" style="width:100%">
            ...
            <div class="col-md-6">
              <button type="button" onclick="them_voucher()" class="btn btn-primary pull-right add-user" id="popup-btn" >
      ...
      <div class="rowa">
        ...
        <thead>
          ...
          <th width="15%">Giá / % áp dụng</th>
          <th width="5%">Số lượng</th>
          <th width="20%">Ngày áp dụng</th>
          <th width="20%">Thời gian áp dụng</th>
        ...
        <tbody>
          ...
          if($time_start != '' && $time_end == ''){
            $sql = $this->Generals_model->notifi('voucher',['created_at >=' => $time_start , 'from' => 2]);
          }
          elseif($time_start == '' && $time_end != ''){
            $sql = $this->Generals_model->notifi('voucher',['created_at <=' => $time_end, 'from' => 2]);
          }
          elseif($time_start != '' && $time_end != ''){
            $sql = $this->Generals_model->notifi('voucher',['created_at >=' => $time_start, 'created_at <=' => $time_end, 'from' => 2]);
          }
          else{
            $sql = $this->Generals_model->notifi('voucher', ['from' => 2]);
          }
          foreach($sql as $key => $v){
          ?>
          <?php
          $a = explode(",", $v["thu_select"]);
          foreach($a as $key => $tach){
            $aa[]= $day[$tach];
          }
          ?>
         <tr>
            ...
            <td><?= date('d-m-Y',$v["start_day"])?> - <?= date('d-m-Y',$v["end_day"])?></td>
            <td><?php
              $a = explode(",", $v["thu_select"]);
              foreach($a as $key => $tach){
                echo  $day[$tach], ",";
            ...
            <td><input type="checkbox" <? if($v['index'] == 1) echo "checked"; ?>  onchange="update_auth(<?= $v['index']?>)"></td>
            <td>
            <a href="administrators-edit-<?=$v['id']?>" class="btn btn-sm btn-success">
                    <i class="fas fa-edit"></i>
                </a>
            </td>
        </tr>
...
<script>
  ...
  function them_voucher(){
      window.location = 'voucher-merchant-add'
  }
  function update_auth(id){
    let data_json = {
        id : id,
    }
    let out = callAjax('/update-auth-admin', 'post', data_json);
    if(out.rs == true){
